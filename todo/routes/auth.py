...
from todo.auth import RefreshToken, Token, authenticate_user, create_access_token, create_refresh_token, validate_token
...
async def refresh_token(form_data: RefreshToken):
    ...
    access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    access_token = create_access_token(
        data={'sub': user.user_name, 'fresh': False}, expires_delta=access_token_expires
    )
    refresh_token_expires = timedelta(minutes=REFRESH_TOKEN_EXPIRE_MINUTES)
    refresh_token = create_refresh_token(
        data={'sub': user.user_name}, expires_delta=refresh_token_expires
